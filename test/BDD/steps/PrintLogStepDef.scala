...
class LogStepDef extends ScalaDsl with EN with Matchers {
	...
	When("""^I put a log request to "/push/([^"]*)" path$""") { (path: String) =>
		val pathUrl = path match {
			case "log" => controllers.routes.Application.putmail.url
	...
	Then("""^I should see the log response http code is ([^"]*) and content printed on screen$""") {(expectedHttpCode: Int) =>
